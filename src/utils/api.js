const api = {
  ...
  getHistoryPrice(token, id, today) {
    return fetch(`${this.hostname}token=${token}&dataset=TaiwanStockPrice&data_id=${id}&start_date='2015-05-15'&end_date=${today}`)
      .then((response) => response.json());
  },
  getStockList(token) {
  ...
  getHistoryDividend(token, id, today) {
    return fetch(`${this.hostname}token=${token}&dataset=TaiwanStockDividend&data_id=${id}&start_date=${this.startDate}&end_date=${today}`)
      .then((response) => response.json());
  },
